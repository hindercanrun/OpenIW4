#pragma once

#include "../stdafx.hpp"
#include "../defs.hpp"

#pragma region LSP Variables
static netadr_t g_iwnetMatchmakingServerAddr = *reinterpret_cast<netadr_t*>(0x1BEAA08);
static netadr_t g_iwnetStorageServerAddr = *reinterpret_cast<netadr_t*>(0x1BEBC40);
static netadr_t stru_66BB078 = *reinterpret_cast<netadr_t*>(0x66BB078);
static netadr_t g_iwnetLoggingServerAddr = *reinterpret_cast<netadr_t*>(0x1BEBC40);

static bool s_logStrings = *reinterpret_cast<bool*>(0x1BEB729);
static bool s_sendStats = *reinterpret_cast<bool*>(0x1BEAEA3);
static bool lsp_connected = *reinterpret_cast<bool*>(0x1BEBC16);
static bool logMsgInittialized = *reinterpret_cast<bool*>(0x66C639A);

static const dvar_t* lsp_debug = *reinterpret_cast<const dvar_t**>(0x66C639C);
#pragma endregion LSP Variables

void LSP_Init();
bool LSP_Connected();
bool LSP_FindTitleServers();
bool LSP_FindTitleServers_f();
void /*__usercall*/ LSP_CheckForLogSend(std::int32_t a1, std::int32_t a2);
void InitLog(std::int32_t a1);
void LSP_LogStringEvenIfControllerIsInactive(const char* string);
void LSP_ForceSendPacket();
std::int32_t Xenon_SendLSPPacket(const char* buf, std::int32_t, netadr_t* net);
void LSP_WritePacketHeader(std::int32_t localControllerIndex, msg_t* msg, std::int32_t a3, std::int32_t a4, char* source);
void LSP_ForceSendPacket();
